<?php
    ...
    if(isset($_POST["AddManager"])){
        ...
        if(!$has_error){
            insertManager($name,$uname,$pass,$email,$number);
        }
        
    }

    if(isset($_POST["EditManager"])){
        updateManager($_POST["id"],$_POST["uname"],$_POST["name"],$_POST["email"],$_POST["phone"],$_POST["salary"]);
    }
    ...
    function insertManager($name,$username,$password,$email,$phone_no){
        $query = "INSERT INTO `users` (`id`, `name`, `username`, `password`, `email`, `phone`, `type`) VALUES (NULL, '$name', '$username', '$password', '$email', '$phone_no', 'manager');";
        execute($query);
        header("Location: AdminDashboard.php");
    }

    function updateManager($id, $uname, $name, $email, $phone_no, $salary){
    ...
    function getAllManager(){
        $query = "SELECT * FROM users WHERE type = 'manager'";
        $result = get($query);
        return $result;
    }

    function getManager($id){
        $query = "SELECT * FROM users WHERE type = 'manager' AND id =$id";
        $result = get($query);
        if(count($result) > 0){
            return $result[0]; 
        }
